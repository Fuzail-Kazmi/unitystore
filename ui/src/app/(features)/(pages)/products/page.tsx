...
// import { useCategories } from "@/api/category"; // 👈 import category hook
...
//                 </div>
//               )}
//               <div className="absolute bottom-2 left-2 flex flex-col gap-2 opacity-0 group-hover:opacity-60 transition-opacity duration-300">
...
const ProductPage = () => {
  ...
  const products: ProductType[] = apiProducts.map((p: ApiProduct) => ({
    id: p.id,
    title: p.product_name,
    brand: p.brand || "Unknown",
    price: p.final_price,
  ...
  const filteredProducts: ProductType[] = products.filter((product) => {
    const matchesCategory = filters.category.length === 0 || (product.category !== null && filters.category.includes(product.category));
    const matchesBrand = filters.brand.length === 0 || filters.brand.includes(product.brand);
    const matchesPrice = product.price >= filters.priceRange[0] && product.price <= filters.priceRange[1];
    const matchesRating = product.rating >= filters.rating;
    const matchesStock = !filters.inStock || product.inStock;
